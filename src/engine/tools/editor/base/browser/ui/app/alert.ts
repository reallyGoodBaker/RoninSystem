...
const [ buttons, setButtons ] = replicable('app.alert.buttons', [
    { text: 'OK', action: 'ok' },
...
export class Alert {
    private static readonly _view = createComputed(() => {
        ...
        return html`
            <div @click=${() => setVisible(false)} class="text-white pop-layer fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50" style="display: ${createComputed(() => visible() ? 'flex' : 'none')}">
                <div @click=${(e: MouseEvent) => e.stopPropagation()} class="outline-1 outline-gray-500 flex flex-col absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-zinc-700 rounded min-h-1/2 min-w-1/2 overflow-clip shadow">
    ...
    static buttons(setVal?: { text: string, action: string }[]) {
        if (setVal === undefined) {
            return buttons()
        }
